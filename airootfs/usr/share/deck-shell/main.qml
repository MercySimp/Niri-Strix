...
// All WebEngineViews share this one profile so the deck_session cookie is
// written to disk and survives app restarts (30-day backend expiry).
WebEngineProfile {
    id: deckProfile
    ...
    // XHR runs in QML's own network stack (no access to the WebEngine cookie
    // jar), so we use a tiny hidden WebEngineView to hit /auth/status and
    // scrape the JSON via runJavaScript. This ensures the persistent
    // deck_session cookie is sent correctly on every cold-start.
    WebEngineView {
        id: statusView
        profile: deckProfile
        visible: false
    ...
    ApplicationWindow {
        ...
        // Legacy XHR logout — still works because the cookie is set by the
        // WebEngine profile on the same origin; logout just clears server-side.
        function doLogout() {
        ...
        Component.onCompleted: {
            // Use the profile-aware status view instead of bare XHR so the
            // persisted deck_session cookie is included in the request.
            statusView.check()
            SteamLibraryCtrl.refresh()
        }
